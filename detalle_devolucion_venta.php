		...
		function BuscarLote() {
			...
			<?php } else { //Editando Devolucion de venta ?>
				remote = open('popup_lotes_sap.php?id=<?php if ($type == 2) {
					...
					echo "0";
				} ?>&edit=<?php echo $type; ?>&objtype=16&sentido=in', 'remote', "width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left=" + posicion_x + ",top=" + posicion_y + "");
				remote.focus();
			<?php } ?>
		}
		...
		function BuscarSerial() {
			...
			<?php } else { //Editando Devolucion de venta ?>
				remote = open('popup_seriales_sap.php?id=<?php if ($type == 2) {
					...
					echo "0";
				} ?>&edit=<?php echo $type; ?>&objtype=16&tipotrans=2', 'remote', "width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left=" + posicion_x + ",top=" + posicion_y + "");
				remote.focus();
			<?php } ?>
		}
	</script>
			...
			<table width="100%" class="table table-bordered">
				<thead>
					<tr>
						...
						<th>Unidad</th>
						<th>Cantidad
							<?php if ($Lotes > 0) { ?><span class="badge badge-info pull-right" title="Ver lotes (Alt+Q)"
									style="cursor: pointer;" onClick="BuscarLote();"><i class="fa fa-tasks"></i></span>
							<?php } ?>
							<?php if ($Seriales > 0) { ?><span class="badge badge-success pull-right"
									title="Ver seriales (Alt+Y)" style="cursor: pointer;" onClick="BuscarSerial();"><i
					...
					if ($sw == 1) {
						...
						while ($row = sqlsrv_fetch_array($SQL)) {
							...
							<tr>
								...
								<td>
									<input size="15" type="text" id="Quantity<?php echo $i; ?>" name="Quantity[]"
										class="form-control"
										...
										onKeyPress="return justNumbers(event,this.value);" <?php if (($row['LineStatus'] == 'C') || ($type == 2)) {
											echo "readonly";
								...
								<td>
									<input size="15" type="text" id="Price<?php echo $i; ?>" name="Price[]" class="form-control"
										...
										onBlur="CalcularTotal(<?php echo $i; ?>);" onKeyUp="revisaCadena(this);"
										onKeyPress="return justNumbers(event,this.value);" <?php if (($row['LineStatus'] == 'C') || ($type == 2) || !PermitirFuncion(420)) {
											echo "readonly";
										} ?>
										onFocus="focalizarValores(this)">
								</td>
