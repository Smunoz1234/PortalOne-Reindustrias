		...
		<div id="page-wrapper" class="gray-bg">
							...
							<form action="consultas_sap.php" method="get" id="formInforme" class="form-horizontal">
								<div class="form-group">
									<label class="col-lg-12">
										<h3 class="bg-success p-xs b-r-sm"><i class="fa fa-check-square-o"></i>
											Críterios de selección</h3>
									</label>
								</div>
								...
								<?php while ($row_Entrada = sqlsrv_fetch_array($SQL_Entradas)) { ?>
									...
									<?php if ($row_Entrada['TipoCampo'] == "Texto") { ?>
										<div class="col-lg-4">
											<label class="control-label">
												<?php echo $row_Entrada['EtiquetaEntrada']; ?>
												<?php if ($row_Entrada['Obligatorio'] == "Y") { ?><span
									...
									<?php } elseif ($row_Entrada['TipoCampo'] == "Cliente") { ?>
										<div class="col-lg-4">
											<label class="control-label"><i
													onClick="ConsultarCliente('<?php echo $row_Entrada['ParametroEntrada']; ?>');"
											...
											<input type="hidden" name="<?php echo $row_Entrada['ParametroEntrada']; ?>"
												id="<?php echo $row_Entrada['ParametroEntrada']; ?>" value="">
									...
									<?php } elseif ($row_Entrada['TipoCampo'] == "Sucursal") { ?>
										<div class="col-lg-4">
											...
											<select
												class="form-control select2 <?php echo ($row_Entrada['Multiple'] == "Y") ? "sucursal-multiple" : "sucursal"; ?>"
												id="<?php echo $row_Entrada['ParametroEntrada']; ?>"
												...
												<?php if ($row_Entrada['Multiple'] == "N") { ?><option value="">(Todos)</option>
												<?php } ?>
											</select>
										</div>
									<?php } elseif ($row_Entrada['TipoCampo'] == "Seleccion") { ?>
										<div class="col-lg-4">
											...
											<select class="form-control" id="<?php echo $row_Entrada['ParametroEntrada']; ?>"
												...
												<option value="N" <?php if (isset($_GET[$row_Entrada['ParametroEntrada']]) && ($_GET[$row_Entrada['ParametroEntrada']] == "N")) {
													echo "selected";
												} ?>>NO
												</option>
											</select>
									...
									<?php } elseif ($row_Entrada['TipoCampo'] == "Lista") { ?>
										...
										<div class="col-lg-4">
											...
											<select class="form-control select2" <?php if ($row_Entrada['Multiple'] == "Y") { ?>data-placeholder="Seleccione..." <?php } ?>
												id="<?php echo $row_Entrada['ParametroEntrada']; ?>"
												name="<?php if ($row_Entrada['Multiple'] == "Y") {
													echo $row_Entrada['ParametroEntrada'] . "[]";
												} else {
													echo $row_Entrada['ParametroEntrada'];
												} ?>"
												<?php if ($row_Entrada['Obligatorio'] == "Y") { ?>required="required" <?php } ?>
												<?php if ($row_Entrada['Multiple'] == "Y") { ?>multiple="multiple" <?php } ?>>
									...
									<?php } elseif ($row_Entrada['TipoCampo'] == "Usuario") { ?>
										<div class="col-lg-4">
											...
											<input name="<?php echo $row_Entrada['ParametroEntrada']; ?>"
												id="<?php echo $row_Entrada['ParametroEntrada']; ?>" type="text"
												class="form-control" value="<?php echo strtolower($_SESSION['User']); ?>"
												readonly>
										</div>
									<?php } ?>
						...
						<div class="form-group">
							<div class="col-lg-4">
								...
								<button type="submit" name="submit" id="submit"
									class="btn btn-success btn-outline pull-right"><i class="fa fa-search"></i>
									Consultar</button>
							</div>
						</div>
			...
			<?php if (isset($_GET['type']) && ($_GET['type'] == "1") && ($testMode == false)) { ?>
				...
				if($SQL_TablaConsulta) {
					...
					echo "$ProcedimientoConsulta<br>";
					echo print_r($ProcedimientoEntradas, true);
				} ?>
							...
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-hover dataTables-example"
									...
									<tbody>
										...
										<?php } while ($SQL_TablaConsulta && ($row_Consulta = sqlsrv_fetch_array($SQL_TablaConsulta))); ?>
									</tbody>
								</table>
							</div> <!-- table -->
						</div> <!-- ibox-content -->
