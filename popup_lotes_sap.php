					...
					<form action="popup_agregar_area.php" method="post" class="form-horizontal" id="FrmAgregar">
						<div class="form-group">
							<label class="col-xs-12">
								<h3 class="bg-muted p-xs b-r-sm"><i class="fa fa-list"></i> Articulos del documento</h3>
							</label>
						</div>
							...
							<tbody>
								...
								while ($row = sqlsrv_fetch_array($SQL_Items)) {
									...
									<tr style="cursor: pointer;"
										onClick="BuscarLote('<?php echo $row['ItemCode']; ?>','<?php echo $row['WhsCode']; ?>','<?php echo $row['LineNum']; ?>','<?php echo base64_encode($row['ItemName']); ?>','<?php echo $row['UnitMsr']; ?>','<?php echo number_format($row['Quantity'], 0); ?>','<?php echo $row['BaseType']; ?>','<?php echo $row['BaseEntry']; ?>','<?php echo $row['BaseLine']; ?>');">
										<td>
											<?php echo ($row['LineNum'] + 1); ?>
										</td>
										<td>
										...
										<td>
											<?php echo $row['UnitMsr']; ?>
										</td>
										<td>
											<?php echo number_format($row['Quantity'], 0); ?>
										</td>
										<td>
											<?php echo $row['WhsName']; ?>
										</td>
									</tr>
									<?php
									$i++;
						...
						<div class="form-group">
							<div class="col-lg-9">
								<button class="btn btn-primary" type="button" form="FrmAgregar"
									onClick="javascript:window.close();" id="Crear"><i class="fa fa-check"></i>
									Aceptar</button>
							</div>
						</div>
					</form>
