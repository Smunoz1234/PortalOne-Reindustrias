			...
			if (dato != "") {
				if ((cardcode != "") && (almacen != "") && (idlistaprecio != "")) {
					remote = open('buscar_articulo.php?dato=' + dato + '&prjcode=' + proyecto + '&empventas=' + empleado + '&cardcode=' + cardcode + '&whscode=' + almacen + '&idlistaprecio=' + idlistaprecio + '&doctype=<?php if ($edit == 0) {
						...
						echo "14";
					} ?>&iddevolucionventa=<?php if ($edit == 1) {
						echo base64_encode($row['ID_DevolucionVenta']);
					} else {
						echo "0";
						...
						echo "0";
					} ?>&tipodoc=2&dim1=' + dim1 + '&dim2=' + dim2 + '&dim3=' + dim3 + '&dim4=' + dim4 + '&dim5=' + dim5, 'remote', "width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left=" + posicion_x + ",top=" + posicion_y + "");
					remote.focus();
				} else {
					Swal.fire({
						title: "¡Advertencia!",
		...
		<div id="page-wrapper" class="gray-bg">
			...
			<div class="wrapper wrapper-content">
								...
								<div class="form-group">
									<div class="col-lg-6">
										...
										<a href="#" class="btn btn-outline btn-info"
											onClick="VerMapaRel('<?php echo base64_encode($row['DocEntry']); ?>','<?php echo base64_encode('16'); ?>');"><i
									...
									<div class="col-lg-6">
										<?php if ($row['DocBaseDocEntry'] != "") { ?>
											<a href="entrega_venta.php?id=<?php echo base64_encode($row['DocBaseDocEntry']); ?>&id_portal=<?php echo base64_encode($row['DocBaseIdPortal']); ?>&tl=1"
												target="_blank" class="btn btn-outline btn-primary pull-right m-l-sm"><i
													class="fa fa-mail-reply"></i> Ir a documento base</a>
										<?php } ?>
										<?php if ($row['Cod_Estado'] == 'O') { ?>
											<button type="button"
												onClick="javascript:location.href='actividad.php?dt_DM=1&Cardcode=<?php echo base64_encode($row['CardCode']); ?>&Contacto=<?php echo base64_encode($row['CodigoContacto']); ?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']); ?>&Direccion=<?php echo base64_encode($row['DireccionDestino']); ?>&DM_type=<?php echo base64_encode('16'); ?>&DM=<?php echo base64_encode($row['DocEntry']); ?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('entrega_venta.php'); ?>'"
				...
				<div class="ibox-content">
					...
					<div class="row">
						<div class="col-lg-12">
							<form action="devolucion_venta.php" method="post" class="form-horizontal"
								enctype="multipart/form-data" id="CrearDevolucionVenta">
								<?php
								$_GET['obj'] = "16";
								...
								<div class="col-lg-4">
									...
									<div class="form-group">
										<label class="col-lg-5">Fecha de devolucion/servicio <span
												class="text-danger">*</span></label>
										<div class="col-lg-7 input-group date">
											<span class="input-group-addon"><i class="fa fa-calendar"></i></span><input
										...
										<div id="tab-1" class="tab-pane active">
											<iframe id="DataGrid" name="DataGrid" style="border: 0;" width="100%"
												...
												src="<?php if ($edit == 0 && $sw_error == 0) {
													echo "detalle_devolucion_venta.php";
												} elseif ($edit == 0 && $sw_error == 1) {
													echo "detalle_devolucion_venta.php?id=0&type=1&usr=" . $_SESSION['CodUser'] . "&cardcode=" . $row['CardCode'];
												} else {
													echo "detalle_devolucion_venta.php?id=" . base64_encode($row['ID_DevolucionVenta']) . "&evento=" . base64_encode($row['IdEvento']) . "&docentry=" . base64_encode($row['DocEntry']) . "&type=2&status=" . base64_encode($row['Cod_Estado']);
												} ?>"></iframe>
										</div>
										<?php if ($edit == 1) { ?>
					...
					<form id="frm" action="" class="form-horizontal">
						...
						<div class="col-lg-8">
							<div class="form-group">
								...
								<div class="col-lg-5">
									<select name="EmpleadoVentas" class="form-control" id="EmpleadoVentas"
										form="CrearDevolucionVenta" required="required" <?php if (($edit == 1) && ($row['Cod_Estado'] == 'C')) {
											echo "disabled='disabled'";
										} ?>>
							...
							<div class="form-group">
								...
								<div class="col-lg-10">
									<textarea name="Comentarios" form="CrearDevolucionVenta" rows="4"
						...
						<div class="col-lg-4">
							<div class="form-group">
								...
								<div class="col-lg-5">
									<input type="text" name="SubTotal" form="CrearDevolucionVenta" id="SubTotal"
							...
							<div class="form-group">
								...
								<div class="col-lg-5">
									<input type="text" name="Descuentos" form="CrearDevolucionVenta" id="Descuentos"
										class="form-control" style="text-align: right; font-weight: bold;"
										value="<?php if ($edit == 1) {
							...
							<div class="form-group">
								...
								<div class="col-lg-5">
									<input type="text" name="Impuestos" form="CrearDevolucionVenta" id="Impuestos"
										class="form-control" style="text-align: right; font-weight: bold;"
										value="<?php if ($edit == 1) {
							...
							<div class="form-group">
								...
								<div class="col-lg-5">
									<input type="text" name="TotalDevolucion" form="CrearDevolucionVenta"
										id="TotalDevolucion" class="form-control"
						...
						<div class="form-group">
							<div class="col-lg-9">
								<?php if ($edit == 0 && PermitirFuncion(409)) { ?>
									<button class="btn btn-primary" type="submit" form="CrearDevolucionVenta" id="Crear"><i
											class="fa fa-check"></i> Crear Devolucion de venta</button>
								<?php } elseif ($row['Cod_Estado'] == "O" && PermitirFuncion(409)) { ?>
									<button class="btn btn-warning" type="submit" form="CrearDevolucionVenta"
										id="Actualizar"><i class="fa fa-refresh"></i> Actualizar Devolucion de
										venta</button>
								<?php } ?>
								<?php
						...
						<input type="hidden" form="CrearDevolucionVenta" id="IdDevolucionVenta" name="IdDevolucionVenta"
							value="<?php if ($edit == 1) {
								echo base64_encode($row['ID_DevolucionVenta']);
							} ?>" />
						<input type="hidden" form="CrearDevolucionVenta" id="IdEvento" name="IdEvento"
							value="<?php if ($edit == 1) {
								echo base64_encode($IdEvento);
							} ?>" />
						<input type="hidden" form="CrearDevolucionVenta" id="d_LS" name="d_LS"
							value="<?php echo $dt_LS; ?>" />
						<input type="hidden" form="CrearDevolucionVenta" id="tl" name="tl"
							value="<?php echo $edit; ?>" />
						<input type="hidden" form="CrearDevolucionVenta" id="swError" name="swError"
							value="<?php echo $sw_error; ?>" />
						<input type="hidden" form="CrearDevolucionVenta" id="return" name="return"
			...
			if (((cardCode != "") && (serie != "")) || probarModal) {
				$.ajax({
					...
					data: {
						Procedure: 35,
						Edit: <?php echo $edit; ?>,
						DocType: "<?php echo ($edit == 0) ? 13 : 14; ?>",
						DocId: "<?php echo $row['ID_DevolucionVenta'] ?? 0; ?>",
