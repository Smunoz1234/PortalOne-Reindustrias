			...
			<div class="row wrapper border-bottom white-bg page-heading">
				...
				<div class="col-sm-4">
					<div class="title-action">
						<a href="usuarios.php" class="btn btn-primary"><i class="fa fa-plus-circle"></i> Agregar
							usuario</a>
					</div>
				</div>
			</div>
									...
									<div class="col-lg-2">
										<select name="BuscarPerfil" class="form-control m-b" id="BuscarPerfil">
											...
											<?php while ($row_Perfiles = sqlsrv_fetch_array($SQL_Perfiles)) { ?>
												<option value="<?php echo $row_Perfiles['ID_PerfilUsuario']; ?>" <?php if ((isset($_GET['BuscarPerfil'])) && (strcmp($row_Perfiles['ID_PerfilUsuario'], $_GET['BuscarPerfil']) == 0)) {
									...
									<div class="col-lg-4">
										<div class="input-group form-group has-success">
											<input name="Buscar" type="text" class="form-control" id="Buscar"
												...
												value="<?php if (isset($_GET['Buscar']) && ($_GET['Buscar'] != "")) {
													echo $_GET['Buscar'];
												} ?>"><span
										...
										<tbody>
											<?php while ($row = sqlsrv_fetch_array($SQL)) { ?>
												<tr>
													...
													<td><?php $Cant = ContarClienteUsuario($row['ID_Usuario']);
													if ($Cant > 0) {
														echo $Cant;
													} else { ?>
															<p class="text-muted">--</p><?php } ?>
													</td>
													<td><?php if ($row['FechaUltIngreso'] != "") {
														echo $row['FechaUltIngreso']->format('Y-m-d H:i:s');
													} else {
														echo "Nunca ha ingresado";
													}
													; ?>
													</td>
													<td id="rowAct<?php echo $row['ID_Usuario']; ?>">
														<?php if ($row['Estado'] == 1) { ?><span
																class="badge badge-primary"><?php echo $row['NombreEstado']; ?></span><?php } else { ?><span
																class="badge badge-danger"><?php echo $row['NombreEstado']; ?></span><?php } ?>
													</td>
													<td>
														<a href="usuarios.php?id=<?php echo base64_encode($row['ID_Usuario']); ?>&tl=1&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('gestionar_usuarios.php'); ?>"
															class="btn btn-info btn-circle" title="Editar"><i
																class="fa fa-edit"></i></a>
														<?php if ($row['Estado'] == 1) { ?>
															<a href="#" id="LinkActive<?php echo $row['ID_Usuario']; ?>"
																onClick="Activar_Inactivar(<?php echo $row['ID_Usuario']; ?>);"
																style="text-decoration:none" title="Inactivar"><button
																	id="btnActive<?php echo $row['ID_Usuario']; ?>" type="button"
																	class="btn btn-danger btn-circle"><span
																		id="spanActive<?php echo $row['ID_Usuario']; ?>"
																		class="glyphicon glyphicon-ban-circle"
																		aria-hidden="true"></span></button></a>
														<?php } else { ?>
															<a href="#" id="LinkActive<?php echo $row['ID_Usuario']; ?>"
																onClick="Activar_Inactivar(<?php echo $row['ID_Usuario']; ?>);"
																style="text-decoration:none" title="Activar"><button
																	id="btnActive<?php echo $row['ID_Usuario']; ?>" type="button"
