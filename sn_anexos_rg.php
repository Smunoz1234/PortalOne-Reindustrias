				...
				<ul class="folder-list" style="padding: 0">
					<?php while ($row_Anexo = sqlsrv_fetch_array($SQL_Anexo)) {
						...
						<li><a href="attachdownload.php?file=<?php echo base64_encode($row_Anexo['AbsEntry']); ?>&line=<?php echo base64_encode($row_Anexo['Line']); ?>"
								target="_blank" class="btn-link btn-xs"><i class="<?php echo $Icon; ?>"></i>
								<?php echo $row_Anexo['NombreArchivo']; ?></a></li>
					<?php } ?>
				</ul>
			</div>
		</div>
...
<?php if ($PedirAnexo == "SI") { ?>
	...
	<div class="form-group">
		<div class="col-lg-4">
			<button class="btn btn-primary" type="button" id="Firmar" onClick="AbrirFirma();"><i
					class="fa fa-pencil-square-o"></i> Realizar firma</button>
			<input type="hidden" id="SigCliente" name="SigCliente" form="EditarSN" value="" />
			<div id="msgInfoSig" style="display: none;" class="alert alert-info"><i class="fa fa-info-circle"></i> El
