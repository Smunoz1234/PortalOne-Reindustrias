...
if ($_SESSION['Perfil'] == 6) {//Perfil de proveedor
...
$sw_Informe = isset($_GET['id']) ? 1 : 0;//Indica si la pagina se esta cargando inicialmente o como informe. 1: Como Informe.
$FechaInicial = '';
$FechaFinal = '';
				...
				<div class="modal-dialog modal-lg">
					<div class="modal-content">
						<div class="modal-header">
							...
							<small>Por favor lea atentamente este contrato que contiene los Términos y Condiciones de
								uso de este sitio. Si continua usando este portal, consideramos que usted está de
								acuerdo con ellos.</small>
						</div>
						...
						<div class="modal-footer">
							<button type="button" onClick="AceptarAcuerdo();" class="btn btn-primary"
								data-dismiss="modal">Acepto los términos</button>
						</div>
					</div>
				</div>
			</div>
							...
							<div class="ibox-content">
								...
								<form action="index1.php" method="get" id="formBuscar" class="form-horizontal">
										...
										<div class="col-lg-2">
											<select name="Sucursal" class="form-control" id="Sucursal">
												...
												<?php while ($row_Sucursal = sqlsrv_fetch_array($SQL_Sucursal)) { ?>
													<option value="<?php echo $row_Sucursal['IdSucursal']; ?>" <?php if (isset($_GET['Sucursal']) && (strcmp($row_Sucursal['IdSucursal'], $_GET['Sucursal']) == 0)) {
														  echo "selected=\"selected\"";
													  } ?>>
														<?php echo $row_Sucursal['DeSucursal']; ?></option>
												<?php } ?>
											</select>
										</div>
									...
									<input type="hidden" name="id" id="id"
										value="<?php echo base64_encode($sw_Informe); ?>" />
								</form>
							</div>
						</div>
					</div>
				<?php } ?>
				<?php if (($sw_Informe == 0) || ($sw_Informe == 1 && $sw == 1)) { ?>
					<div class="row">
						<div class="col-lg-3">
							<div class="ibox ">
								<div class="ibox-title"> <span
										class="label label-info pull-right"><?php if ($sw_Informe == 0) {
											echo ucwords(strftime("%B"));
										} ?></span>
									<h5>Llamadas abiertas</h5>
								</div>
								<div class="ibox-content">
									<h1 class="no-margins"><?php echo number_format($row_Llamadas['LlamadasAbiertas'], 0); ?>
									</h1>
								</div>
							</div>
						</div>
						<div class="col-lg-3">
							<div class="ibox ">
								<div class="ibox-title"> <span
										class="label label-warning pull-right"><?php if ($sw_Informe == 0) {
											echo ucwords(strftime("%B"));
										} ?></span>
								...
								<div class="ibox-content">
									<h1 class="no-margins">
										<?php echo number_format($row_Llamadas['LlamadasPendientes'], 0); ?></h1>
								</div>
							</div>
						</div>
						<div class="col-lg-3">
							<div class="ibox ">
								<div class="ibox-title"> <span
										class="label label-danger pull-right"><?php if ($sw_Informe == 0) {
											echo ucwords(strftime("%B"));
										} ?></span>
									<h5>Llamadas cerradas</h5>
								</div>
								<div class="ibox-content">
									<h1 class="no-margins"><?php echo number_format($row_Llamadas['LlamadasCerradas'], 0); ?>
									</h1>
								</div>
							</div>
						</div>
						<div class="col-lg-3">
							<div class="ibox ">
								<div class="ibox-title"> <span
										class="label label-success pull-right"><?php if ($sw_Informe == 0) {
											echo ucwords(strftime("%B"));
										} ?></span>
									<h5>Total llamadas</h5>
								</div>
								<div class="ibox-content">
									<h1 class="no-margins"><?php echo number_format($row_Llamadas['TotalLlamadas'], 0); ?>
									</h1>
								</div>
							</div>
						</div>
										...
										<div class="tabs-container">
											<ul class="nav nav-tabs">
												<li class="active"><a data-toggle="tab" href="#tab-1"><i
															class="fa fa-calendar"></i> Recibidas <span
															class="label label-success float-right m-l-sm"><?php echo $Num_MisAct; ?></span></a>
												</li>
												<li><a data-toggle="tab" href="#tab-2"><i class="fa fa-calendar"></i> Enviadas
														<span
															class="label label-success float-right m-l-sm"><?php echo $Num_ActAsig; ?></span></a>
												</li>
											</ul>
										</div>
													...
													if ($Num_MisAct > 0) {
														while ($row_MisAct = sqlsrv_fetch_array($SQL_MisAct)) {
															...
															<div class="feed-element">
																<div>
																	<div class="col-xs"> <a class="col-xs-6"
																			href="actividad.php?id=<?php echo base64_encode($row_MisAct['ID_Actividad']); ?>&tl=1"
																			target="_blank"> <span
																				class="badge badge-primary"><?php echo $row_MisAct['ID_Actividad']; ?></span>
																		</a> <span class="pull-right badge badge-success font-bold"><i
																				class="fa fa-user"></i>
																			<?php echo $row_MisAct['DeAsignadoPor']; ?></span> </div>
																	<div class="col-xs">
																		<h4 class="col-xs-12 font-bold no-margins">
																			<?php echo $row_MisAct['DE_AsuntoActividad']; ?></h4>
																	</div>
																	<div class="col-xs">
																		<h4 class="col-xs-6 font-bold no-margins">
																			<?php echo $row_MisAct['DE_TipoActividad']; ?></h4>
																		<h4 class="pull-right no-margins <?php echo $DVenc[0]; ?>">
																			<strong>Vencimiendo: </strong>
																			<?php if ($DVenc[1] == 0) {
																				echo "Hoy";
																			} else {
																				echo $DVenc[1] . " dias";
																	...
																	<div class="col-xs">
																		<h4 class="col-xs-6 text-navy no-margins">Inicia:
																			<?php echo $row_MisAct['FechaHoraInicioActividad']->format('Y-m-d H:s'); ?>
																		</h4>
																		<h4 class="pull-right text-danger no-margins">Finaliza:
																			<?php echo $row_MisAct['FechaHoraFinActividad']->format('Y-m-d H:s'); ?>
																		</h4>
																	</div>
																</div>
															</div>
													...
													<?php if ($Num_ActAsig > 0) {
														while ($row_ActAsig = sqlsrv_fetch_array($SQL_ActAsig)) {
															...
															<div class="feed-element">
																<div>
																	<div class="col-xs"> <a class="col-xs-6"
																			href="actividad.php?id=<?php echo base64_encode($row_ActAsig['ID_Actividad']); ?>&tl=1"
																			target="_blank"> <span
																				class="badge badge-primary"><?php echo $row_ActAsig['ID_Actividad']; ?></span>
																		</a> <span class="pull-right badge badge-success font-bold"><i
																				class="fa fa-user"></i>
																			<?php echo $row_ActAsig['NombreEmpleado']; ?></span> </div>
																	<div class="col-xs">
																		<h4 class="col-xs-12 font-bold no-margins">
																			<?php echo $row_ActAsig['DE_AsuntoActividad']; ?></h4>
																	</div>
																	<div class="col-xs">
																		<h4 class="col-xs-6 font-bold no-margins">
																			<?php echo $row_ActAsig['DE_TipoActividad']; ?></h4>
																		<h4 class="pull-right no-margins <?php echo $DVenc[0]; ?>">
																			<strong>Vencimiendo: </strong>
																			<?php if ($DVenc[1] == 0) {
																				echo "Hoy";
																	...
																	<div class="col-xs">
																		<h4 class="col-xs-6 text-navy no-margins">Inicia:
																			<?php echo $row_ActAsig['FechaHoraInicioActividad']->format('Y-m-d H:s'); ?>
																		</h4>
																		<h4 class="pull-right text-danger no-margins">Finaliza:
																			<?php echo $row_ActAsig['FechaHoraFinActividad']->format('Y-m-d H:s'); ?>
																		</h4>
																	</div>
																</div>
															</div>
		...
		<script>
			Highcharts.chart('graph_4', {
				...
				xAxis: {
					...
					categories: [<?php for ($i = 0; $i < count($dias); $i++) {
						echo "'" . $dias[$i] . "',";
					} ?>]
				},
				yAxis: {
