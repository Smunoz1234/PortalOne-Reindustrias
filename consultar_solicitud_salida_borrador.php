...
<head>
	...
	<title>Consultar solicitud de traslado borrador |
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="ibox-content">
							...
							<form action="consultar_solicitud_salida_borrador.php" method="get" id="formBuscar"
											...
											<?php if (isset($_GET['Series']) && ($_GET['Series'] != "")) {
												while ($row_Sucursal = sqlsrv_fetch_array($SQL_Sucursal)) { ?>
													<option value="<?php echo $row_Sucursal['IdSucursal']; ?>" <?php if (isset($_GET['Sucursal']) && (strcmp($row_Sucursal['IdSucursal'], $_GET['Sucursal']) == 0)) {
														...
														<?php echo $row_Sucursal['DeSucursal']; ?>
													</option>
												<?php }
											} ?>
										</select>
									...
									<div class="col-lg-3">
										<select name="Empleado" class="form-control select2" id="Empleado">
											...
											<?php while ($row_Empleado = sqlsrv_fetch_array($SQL_Empleado)) { ?>
												<option value="<?php echo $row_Empleado['ID_Empleado']; ?>" <?php if ((isset($_GET['Empleado'])) && (strcmp($row_Empleado['ID_Empleado'], $_GET['Empleado']) == 0)) {
													   echo "selected=\"selected\"";
												   } ?>>
									...
									<div class="col-lg-3">
										<select name="TipoEntrega" class="form-control" id="TipoEntrega">
											...
											<?php while ($row_TipoEntrega = sqlsrv_fetch_array($SQL_TipoEntrega)) { ?>
												<option value="<?php echo $row_TipoEntrega['IdTipoEntrega']; ?>" <?php if ((isset($_GET['TipoEntrega'])) && (strcmp($row_TipoEntrega['IdTipoEntrega'], $_GET['TipoEntrega']) == 0)) {
													   echo "selected=\"selected\"";
												   } ?>>
										...
										<div class="col-lg-2">
											<select name="AnioEntrega" class="form-control" id="AnioEntrega">
												...
												<?php while ($row_AnioEntrega = sqlsrv_fetch_array($SQL_AnioEntrega)) { ?>
													<option value="<?php echo $row_AnioEntrega['IdAnioEntrega']; ?>" <?php if ((isset($_GET['AnioEntrega'])) && (strcmp($row_AnioEntrega['IdAnioEntrega'], $_GET['AnioEntrega']) == 0)) {
														   echo "selected=\"selected\"";
													   } ?>>
									...
									<div id="dv_Descont" style="display: none;">
										...
										<div class="col-lg-2">
											<select name="EntregaDescont" class="form-control" id="EntregaDescont">
												<option value="">(Todos)</option>
												<option value="NO" <?php if ((isset($_GET['EntregaDescont'])) && ($_GET['EntregaDescont'] == "NO")) {
													echo "selected=\"selected\"";
												} ?>>
													NO</option>
												<option value="SI" <?php if ((isset($_GET['EntregaDescont'])) && ($_GET['EntregaDescont'] == "SI")) {
													echo "selected=\"selected\"";
												} ?>>
													SI</option>
											</select>
										...
										<?php if ($sw == 1) { ?>
											<?php while ($row = sqlsrv_fetch_array($SQL)) { ?>
												<tr class="gradeX">
													...
													<td>
														<?php echo $row['OcrName3']; ?>
													</td>
													<td>
														<?php echo $row['DocDate']; ?>
													</td>
													<td>
														<?php echo $row['NomEmpleado']; ?>
													</td>
													<td>
														<?php echo $row['DeTipoEntrega']; ?>
													</td>
													...
													<td>
														<?php if (isset($row["DocEntryDocumentoDefinitivo"]) && isset($row["DocNumDocumentoDefinitivo"])) { ?>
															<a target="_blank"
																href="solicitud_salida.php?id=<?php echo base64_encode($row['DocEntryDocumentoDefinitivo']); ?>&id_portal=<?php echo base64_encode($row['DocNumDocumentoDefinitivo']); ?>&tl=1&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('consultar_solicitud_salida_borrador.php'); ?>"
													...
													<td>
														<?php echo SubComent($row['Comentarios']); ?>
													</td> <!-- SMM, 25/11/2022 -->
													<td>
														<?php echo $row['Descontable']; ?>
													</td>
													...
													<td>
														<a href="solicitud_salida_borrador.php?id=<?php echo base64_encode($row['ID_SolSalida']); ?>&id_portal=<?php echo base64_encode($row['IdDocPortal']); ?>&tl=1&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('consultar_solicitud_salida_borrador.php'); ?>"
															class="alkin btn btn-success btn-xs"><i
																class="fa fa-folder-open-o"></i> Abrir</a>
														<a href="sapdownload.php?id=<?php echo base64_encode('15'); ?>&type=<?php echo base64_encode('2'); ?>&DocKey=<?php echo base64_encode($row['ID_SolSalida']); ?>&ObType=<?php echo base64_encode('1250000001'); ?>&IdFrm=<?php echo base64_encode($row['IdSeries']); ?>"
