...
function BuscarArticulo(dato){
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")){
			...
			remote=open(`buscar_articulo.php?borrador=1&concepto=${conceptoSalida}&dim1=${dim1}&dim2=${dim2}&dim3=${dim3}&towhscode=${almacenDestino}&prjcode=${proyecto}&dato=`+dato+'&cardcode='+cardcode+'&whscode='+almacen+'&doctype=<?php if ($edit == 0) {
				echo "7";
			} else {
				echo "8";
												...
												<div class="form-group">
													...
													<div class="col-lg-10">
														<textarea <?php if ($row["AuthPortal"] != "P") {
															echo "readonly";
														} ?> type="text" maxlength="200" rows="4" class="form-control" form="CrearSolicitudSalida" name="ComentariosAutorizacionPO" id="ComentariosAutorizacionPO"><?php if (isset($row_Autorizaciones['ComentariosAutorizador_SAPB1'])) {
						...
						<select name="Autorizacion" class="form-control" id="Autorizacion" readonly>
						  <?php while ($row_EstadoAuth = sqlsrv_fetch_array($SQL_EstadoAuth)) { ?>
								<option value="<?php echo $row_EstadoAuth['IdAuth']; ?>" 
									<?php if ($row_EstadoAuth['IdAuth'] == "N") {
										echo "disabled";
					...
					<div class="tab-content">
						<div id="tab-1" class="tab-pane active">
							<iframe id="DataGrid" name="DataGrid" style="border: 0;" width="100%" height="300" src="<?php if ($edit == 0 && $sw_error == 0) {
								echo "detalle_solicitud_salida_borrador.php";
							} elseif ($edit == 0 && $sw_error == 1) {
								echo "detalle_solicitud_salida_borrador.php?id=0&type=1&usr=" . $_SESSION['CodUser'] . "&cardcode=" . $row['CardCode'] . "&whscode=" . $row['WhsCode'];
							} else {
								echo "detalle_solicitud_salida_borrador.php?bloquear=$BloquearDocumento&id=" . base64_encode($row['ID_SolSalida']) . "&evento=" . base64_encode($row['IdEvento']) . "&type=2&status=" . base64_encode($EstadoReal) . "&docentry=" . base64_encode($row['DocEntry']);
							} ?>"></iframe>
						</div>
						<?php if ($edit == 1) { ?>
			   ...
			   <form id="frm" action="" class="form-horizontal">
				...
				<div class="col-lg-8">
					...
					<div class="form-group">
						...
						<div class="col-lg-10">
							<textarea name="Comentarios" form="CrearSolicitudSalida" rows="4" class="form-control" id="Comentarios"><?php if ($edit == 1) {
					...
					<div class="col-lg-9">
						...
						<?php } elseif ($row['Cod_Estado'] == "O" && PermitirFuncion(1201)) { ?>
								...
								<?php if ((strtoupper($_SESSION["User"]) != strtoupper($row['Usuario'])) || $serAutorizador) { ?>
										...
										<button class="btn btn-warning" type="submit" form="CrearSolicitudSalida" id="Actualizar" <?php if (!$autorAsignado) {
											echo "disabled";
										} ?>><i class="fa fa-refresh"></i> Actualizar Solicitud de Traslado Borrador</button>
								<?php } else { ?>
