...
$Cons = "SELECT * FROM [uvw_Sap_tbl_FacturasAnticiposCompras] WHERE $WhereFecha $Filtro ORDER BY DocNum DESC";
...
if (isset($_GET['IDTicket']) && $_GET['IDTicket'] != "") {
	...
	$Cons = "SELECT * FROM [uvw_Sap_tbl_FacturasAnticiposCompras] WHERE $Where";
}
...
<head>
	...
	<title>Consultar Factura anticipo de Compras | <?php echo NOMBRE_PORTAL; ?></title>
	<!-- InstanceEndEditable -->
	...
	if (isset($_GET['a']) && ($_GET['a'] == base64_encode("OK_OCompAdd"))) {
		...
		$(document).ready(function() {
			Swal.fire({
                title: '¡Listo!',
                text: 'La Factura anticipo de Compras ha sido agregada exitosamente.',
                icon: 'success'
            });
		});
		</script>";
	...
	<div id="wrapper">
		...
		<div id="page-wrapper" class="gray-bg">
			...
			<div class="row wrapper border-bottom white-bg page-heading">
				<div class="col-sm-8">
					<h2>Consultar Factura anticipo de Compras</h2>
					<ol class="breadcrumb">
						<li>
						...
						<li class="active">
							<strong>Consultar Factura anticipo de Compras</strong>
						</li>
					</ol>
				</div>
			</div>
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="ibox-content">
							...
							<form action="consultar_factura_anticipo_compra.php" method="get" id="formBuscar"
											...
											<?php while ($row = sqlsrv_fetch_array($SQL)) { ?>
												<tr class="gradeX">
													...
													<td><?php if ($row['ID_LlamadaServicio'] != 0) { ?><a
																href="llamada_servicio.php?id=<?php echo base64_encode($row['ID_LlamadaServicio']); ?>&return=<?php echo base64_encode($_SERVER['QUERY_STRING']); ?>&pag=<?php echo base64_encode('consultar_factura_anticipo_compra.php'); ?>&tl=1"
																target="_blank"><?php echo $row['DocNumLlamadaServicio']; ?></a><?php } else {
														echo "--";
													} ?>
													</td>
													<td><?php if ($row['DocBaseDocEntry'] != "") { ?><a
																href="solicitud_compra.php?id=<?php echo base64_encode($row['DocBaseDocEntry']); ?>&id_portal=<?php echo base64_encode($row['DocBaseIdPortal']); ?>&tl=1"
																target="_blank"><?php echo $row['DocBaseDocNum']; ?></a><?php } else {
														echo "--";
													} ?>
													</td>
													<td><?php if ($row['DocDestinoDocEntry'] != "") { ?><a
																href="entrada_compra.php?id=<?php echo base64_encode($row['DocDestinoDocEntry']); ?>&id_portal=<?php echo base64_encode($row['DocDestinoIdPortal']); ?>&tl=1"
