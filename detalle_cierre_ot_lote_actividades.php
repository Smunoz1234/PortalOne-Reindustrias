					...
					if ($sw == 1) {
						...
						while ($row = sqlsrv_fetch_array($SQL)) {
							...
							<tr>
								...
								<td><input size="15" type="text" id="ID_OrdenServicio<?php echo $i; ?>"
										name="ID_OrdenServicio[]" class="form-control" readonly
										value="<?php echo $row['ID_OrdenServicio']; ?>"></td>
								<td><input size="15" type="text" id="ID_Actividad<?php echo $i; ?>" name="ID_Actividad[]"
										class="form-control" readonly value="<?php echo $row['ID_Actividad']; ?>"><input
										type="hidden" name="ID[]" id="ID<?php echo $i; ?>" value="<?php echo $row['ID']; ?>">
								</td>
								<td><input size="30" type="text" id="NombreEmpleado<?php echo $i; ?>" name="NombreEmpleado[]"
										class="form-control" readonly value="<?php echo $row['NombreEmpleadoActividad']; ?>">
								</td>
								<td><input size="25" type="text" id="FechaHoraInicioActividad<?php echo $i; ?>"
										name="FechaHoraInicioActividad[]" class="form-control" readonly
										value="<?php echo $row['FechaHoraInicioActividad']->format('Y-m-d H:i'); ?>"></td>

								<td><input size="25" type="text" id="FechaHoraInicioEjecucion<?php echo $i; ?>"
										name="FechaHoraInicioEjecucion[]" class="form-control FechaHoraInicioEjecucion"
										onChange="ActualizarDatos('FechaHoraInicioEjecucion',<?php echo $i; ?>,<?php echo $row['ID']; ?>);"
										data-mask="9999-99-99 99:99" value="<?php
										// SMM, 13/09/2022
										if (isset($_GET["FechaInicioEjecucion"]) && ($_GET["FechaInicioEjecucion"] != "") && isset($_GET["HoraInicioEjecucion"]) && ($_GET["HoraInicioEjecucion"] != "")) {
											...
											if ($row['FechaHoraInicioEjecucion'] != "") {
												echo $row['FechaHoraInicioEjecucion']->format('Y-m-d H:i');
											}
										} ?>"></td>

								<td><input size="25" type="text" id="FechaHoraFinEjecucion<?php echo $i; ?>"
										name="FechaHoraFinEjecucion[]" class="form-control FechaHoraFinEjecucion"
										onChange="ActualizarDatos('FechaHoraFinEjecucion',<?php echo $i; ?>,<?php echo $row['ID']; ?>);"
										data-mask="9999-99-99 99:99" value="<?php
										// SMM, 13/09/2022
										if (isset($_GET["FechaFinEjecucion"]) && ($_GET["FechaFinEjecucion"] != "") && isset($_GET["HoraFinEjecucion"]) && ($_GET["HoraFinEjecucion"] != "")) {
											...
											if ($row['FechaHoraFinEjecucion'] != "") {
												echo $row['FechaHoraFinEjecucion']->format('Y-m-d H:i');
											}
										} ?>"></td>

								<td><input size="15" type="text" id="EstadoActividad<?php echo $i; ?>" name="EstadoActividad[]"
										class="form-control <?php if ($row['EstadoActividad'] == "Abierto") {
											echo "bg-danger";
										} else {
											echo "bg-primary";
										} ?>"
										readonly value="<?php echo $row['EstadoActividad']; ?>"></td>
