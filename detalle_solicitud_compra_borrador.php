			...
			<table width="100%" class="table table-bordered">
				...
				<tbody>
					...
					if ($sw == 1) {
								...
								<?php // Inicio, comprobando permiso 717.
									...
									<td class="text-center form-inline w-150">
										<div class="checkbox checkbox-success"><input type="checkbox" class="chkSel"
												...
												aria-label="Single checkbox One" <?php if (($flag && ($row['TreeType'] == "T")) || ($row['LineStatus'] == 'C') || $bandera_autorizacion) {
													echo "disabled";
												} ?>><label></label></div>
									...
									<td><input size="50" type="text" autocomplete="off" id="ItemName<?php echo $i; ?>"
											...
											<?php if (($row['LineStatus'] == 'C') || $bandera_autorizacion || (!PermitirFuncion(702))) {
												echo "readonly";
											} ?>></td>
									...
									<td>
										<input size="15" type="text" autocomplete="off" id="Quantity<?php echo $i; ?>"
											...
											onKeyPress="return justNumbers(event,this.value);" <?php if (($row['LineStatus'] == 'C') || $bandera_autorizacion || (isset($row['TrgetEntry']) && $row['TrgetEntry'] != "") || (!PermitirFuncion(702))) {
												echo "readonly";
									...
									<td> <!-- SMM, 09/02/2023 -->
										<input type="hidden" class="CardCode" name="CardCode[]" id="CardCode<?php echo $i; ?>"
											value="<?php echo $row['CardCode']; ?>">
										<input class="form-control CardName" data-id="CardCode<?php echo $i; ?>" name="CardName[]"
											id="CardName<?php echo $i; ?>" autocomplete="off" size="50" type="text"
											placeholder="Digite para buscar..." value="<?php echo $row['CardName']; ?>">
									</td>

									<td> <!-- SMM, 07/02/2023 -->
										<input size="15" type="text" id="ReqDate<?php echo $i; ?>" name="ReqDate[]"
											class="form-control"
											onChange="ActualizarDatos('ReqDate',<?php echo $i; ?>,<?php echo $row['LineNum']; ?>);"
											value="<?php if (isset($row['ReqDate']) && ($row['ReqDate'] != "")) {
												echo $row['ReqDate'];
											} ?>"
											data-mask="9999-99-99" placeholder="AAAA-MM-DD">
									</td>
									...
									<!-- Nuevos campos basados en la OT. SMM, 19/04/2024 -->
									<td>
									...
									<td>
										<select name="CDU_IdTipoCargo[]" id="CDU_IdTipoCargo<?php echo $i; ?>"
											...
											<?php while ($row_TIPOCARGO = sqlsrv_fetch_array($SQL_OT_TIPOCARGO)) { ?>
												<option value="<?php echo $row_TIPOCARGO['IdTipoCargo']; ?>" <?php if ((isset($row["CDU_IdTipoCargo"])) && (strcmp($row_TIPOCARGO['IdTipoCargo'], $row["CDU_IdTipoCargo"]) == 0)) {
														echo "selected";
													} ?>>
									...
									<td><input size="50" type="text" id="FreeTxt<?php echo $i; ?>" name="FreeTxt[]"
											...
											maxlength="100" <?php if ($row['LineStatus'] == 'C' || (!PermitirFuncion(702))) {
												echo "readonly";
											} ?>></td>
									...
									<td>
										<input size="15" type="text" id="Price<?php echo $i; ?>" name="Price[]" class="form-control"
											...
											onBlur="CalcularTotal(<?php echo $i; ?>);" onKeyUp="revisaCadena(this);"
											onKeyPress="return justNumbers(event,this.value);" <?php if ($row['LineStatus'] == 'C' || (!PermitirFuncion(702)) || !PermitirFuncion(711)) {
												echo "readonly";
											} ?>
											onFocus="focalizarValores(this)">
									</td>
									...
									<td>
										...
										<input type="hidden" id="TarifaIVA<?php echo $i; ?>" name="TarifaIVA[]"
											value="<?php echo number_format($row['TarifaIVA'], 0, $sDecimal, $sMillares); ?>">
										<input type="hidden" id="VatSum<?php echo $i; ?>" name="VatSum[]"
											value="<?php echo number_format($row['VatSum'], 2, $sDecimal, $sMillares); ?>">
									</td>

									<td>
										<input size="15" type="text" id="PriceDisc<?php echo $i; ?>" name="PriceDisc[]"
											class="form-control" value="0" onBlur="CalcularTotal(<?php echo $i; ?>);"
											onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);"
											readonly onFocus="focalizarValores(this)">
									</td>

									<td>
										<input size="15" type="text" id="DiscPrcnt<?php echo $i; ?>" name="DiscPrcnt[]"
											class="form-control"
											value="<?php echo number_format($row['DiscPrcnt'], $dPorcentajes, $sDecimal, $sMillares); ?>"
											...
											onKeyPress="return justNumbers(event,this.value);" <?php if ($row['LineStatus'] == 'C' || (!PermitirFuncion(702)) || (!PermitirFuncion(712))) {
												echo "readonly";
											} ?>
											onFocus="focalizarValores(this)">
									</td>
									...
									<td>
										<input size="15" type="text" id="LineTotal<?php echo $i; ?>" name="LineTotal[]"
											class="form-control"
											...
											onKeyPress="return justNumbers(event,this.value);" <?php if ($row['LineStatus'] == 'C' || (!PermitirFuncion(702))) {
												echo "readonly";
											} ?> autocomplete="off"
											onFocus="focalizarValores(this)">
									</td>
									...
									<td>
										<input type="checkbox" id="ControlDesc<?php echo $i; ?>" name="ControlDesc[]"
											class="form-control"
									...
									<td>
										<input type="checkbox" id="SujetoImpuesto<?php echo $i; ?>" name="SujetoImpuesto[]"
											class="form-control chkExento"
											onChange="ActualizarDatos('SujetoImpuesto',<?php echo $i; ?>, <?php echo $row['LineNum']; ?>);"
											<?php if (isset($row['SujetoImpuesto']) && ($row['SujetoImpuesto'] == "N")) {
												echo "checked";
											} ?> data-exento="<?php echo $row['SujetoImpuesto']; ?>" disabled>
									</td>
						...
						if ($main && !$flag) {
							echo "<script>BorrarLineaPrincipal(); </script>";
						}
