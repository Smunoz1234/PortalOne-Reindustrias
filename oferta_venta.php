...
if (isset($_GET['dt_LS']) && ($_GET['dt_LS']) == 1) { //Verificar que viene de una Llamada de servicio (Datos Llamada servicio)
    ...
    //Sucursal cliente
    $SQL_SucursalDestino = Seleccionar('uvw_Sap_tbl_Clientes_Sucursales', '*', "CodigoCliente=''" . base64_decode($_GET['Cardcode']) . "''", 'NombreSucursal');
...
<script type="text/javascript">
	$(document).ready(function() { // Cargar los combos dependiendo de otros
		...
		$("#Serie").change(function() {
			...
			let Dim2=document.getElementById('Dim2').value; // SMM, 25/04/2022
			$.ajax({
				type: "POST",
		...
		$("#Dim2").change(function(){
			...
			$.ajax({
				type: "POST",
				url: `ajx_cbo_select.php?type=20&id=${Dim2}&serie=${Serie}&tdoc=23&WhsCode=<?php echo isset($_GET['Almacen']) ? base64_decode($_GET['Almacen']) : ($row['WhsCode'] ?? ""); ?>`, // Modificación almacen, SMM/01/05/2022
				success: function(response){
			...
			if(Dim3!=""&&document.getElementById('CardCode').value!=""&&document.getElementById('TotalItems').value!="0"){
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=2&type=1&name=OcrCode3&value="+Base64.encode(Dim3)+"&line=0&cardcode="+document.getElementById('CardCode').value+"&whscode=0&actodos=1",
							success: function(response){
								frame.src="detalle_oferta_venta.php?id=0&type=1&usr=<?php echo $_SESSION['CodUser']; ?>&cardcode="+document.getElementById('CardCode').value;
								$('.ibox-content').toggleClass('sk-loading',false);
							}
						...
						$.ajax({
							type: "GET",
							url: "registro.php?P=36&doctype=2&type=2&name=OcrCode3&value="+Base64.encode(Dim3)+"&line=0&id=<?php echo $row['ID_OfertaVenta']; ?>&evento=<?php echo $IdEvento; ?>&actodos=1",
							success: function(response){
								frame.src="detalle_oferta_venta.php?id=<?php echo base64_encode($row['ID_OfertaVenta']); ?>&evento=<?php echo base64_encode($IdEvento); ?>&type=2";
								$('.ibox-content').toggleClass('sk-loading',false);
							}
					...
					<div class="form-group">
						...
						<div class="col-lg-11">
							<select name="OrdenServicioCliente" class="form-control select2" id="OrdenServicioCliente" <?php if (($edit == 1) && ($row['Cod_Estado'] == 'C')) {echo "disabled='disabled'";}?>>
								...
								<?php }?>
								<?php if ($edit == 1 || $dt_LS == 1 || $sw_error == 1) {?>
									<?php while ($row_OrdenServicioCliente = sqlsrv_fetch_array($SQL_OrdenServicioCliente)) {?>
							...
							<div class="col-lg-3">
								<div class="btn-group pull-right">
									...
									<ul class="dropdown-menu">
										...
										<li><a class="alkin dropdown-item d-venta" href="oferta_venta.php?dt_OF=1&Cardcode=<?php echo base64_encode($row['CardCode']); ?>&Dim1=<?php echo base64_encode($row['OcrCode']); ?>&Dim2=<?php echo base64_encode($row['OcrCode2']); ?>&Dim3=<?php echo base64_encode($row['OcrCode3']); ?>&Sucursal=<?php echo base64_encode($row['SucursalDestino']); ?>&Direccion=<?php echo base64_encode($row['DireccionDestino']); ?>&Almacen=<?php echo base64_encode($row['WhsCode']); ?>&Contacto=<?php echo base64_encode($row['CodigoContacto']); ?>&Empleado=<?php echo base64_encode($row['SlpCode']); ?>&OF=<?php echo base64_encode($row['ID_OfertaVenta']); ?>&Evento=<?php echo base64_encode($row['IdEvento']); ?>&dt_LS=1&LS=<?php echo base64_encode($row['ID_LlamadaServicio']); ?>&pag=<?php echo $_GET['pag']; ?>&return=<?php echo $_GET['return']; ?>&Comentarios=<?php echo base64_encode($row['Comentarios']); ?>&Proyecto=<?php echo base64_encode($row['PrjCode']); ?>&CondicionPago=<?php echo base64_encode($row['IdCondicionPago']); ?>&Serie=<?php echo base64_encode($row['IdSeries']); ?>">Oferta de venta (Duplicar)</a></li>
									</ul>
								</div>
							</div>
