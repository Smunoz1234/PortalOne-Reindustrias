...
if (isset($_GET['id']) && ($_GET['id'] != "")) {
    ...
    if ($type == 1) { //Creando Factura de Compra
        $SQL = Seleccionar("uvw_tbl_FacturaCompraDetalleCarrito_Borrador", "*", "Usuario='" . $_GET['usr'] . "' and CardCode='" . $_GET['cardcode'] . "'");
        if ($SQL) {
            $sw = 1;
        ...
        $SQL = Seleccionar("uvw_tbl_FacturaCompraDetalle_Borrador", "*", "ID_FacturaCompra='" . base64_decode($_GET['id']) . "' and IdEvento='" . base64_decode($_GET['evento']) . "' and Metodo <> 3");
        if ($SQL) {
            $sw = 1;
        }
    }
...
function BorrarLinea(){
	if(confirm(String.fromCharCode(191)+'Est'+String.fromCharCode(225)+' seguro que desea eliminar este item? Este proceso no se puede revertir.')){
		$.ajax({
			...
			<?php }?>
			success: function(response){
				window.location.href="detalle_factura_compra_borrador.php?<?php echo $_SERVER['QUERY_STRING']; ?>";
