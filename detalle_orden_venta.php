...
function Totalizar(num, totalizar=true) {
	...
	window.parent.document.getElementById('TotalOrden').value = number_format(Math.floor(Math.round(Total)), 2);
...
if ($sw == 1) {
    ...
    while ($row = sqlsrv_fetch_array($SQL)) {
        ...
        if ($flag || (($row['LineNum'] != 0) || ($row['TreeType'] != "T"))) {?>
			...
			<td>
				<input size="15" type="text" id="PriceTax<?php echo $i; ?>" name="PriceTax[]" class="form-control iva" value="<?php echo number_format($row['PriceTax'], 2); ?>" onBlur="CalcularTotal(<?php echo $i; ?>);" onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);" readonly onFocus="focalizarValores(this)">
