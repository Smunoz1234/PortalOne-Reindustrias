			...
			<table width="100%" class="table table-bordered">
				<thead>
					<tr>
						...
						<th>Abrir <button type="button" title="Mostrar todas las actividades"
								class="btn btn-success btn-xs" onClick="CargarAct('');"><i
						...
						while ($row = sqlsrv_fetch_array($SQL)) {
							...
							<tr>
								...
								<td class="text-center"><button type="button" title="Mostrar actividades"
										...
										onClick="CargarAct('<?php echo $row['ID_Llamada']; ?>');"><i
											class="fa fa-eye"></i></button></td>
								<td><a href="llamada_servicio.php?id=<?php echo base64_encode($row['ID_Llamada']); ?>&tl=1"
										target="_blank" id="OrdenServicio<?php echo $i; ?>"><?php echo $row['ID_OrdenServicio']; ?></a></td>
								<td><input size="50" type="text" id="NombreCliente<?php echo $i; ?>" name="NombreCliente[]"
										class="form-control" readonly value="<?php echo $row['NombreCliente']; ?>"></td>
								<td><input size="50" type="text" id="SucursalCliente<?php echo $i; ?>" name="SucursalCliente[]"
										class="form-control" readonly value="<?php echo $row['IdSucursalCliente']; ?>"></td>
								<td>
									<select id="EstadoServicio<?php echo $i; ?>" name="EstadoServicio[]"
										class="form-control IdEstadoServicio"
										onChange="ActualizarDatos('EstadoServicio',<?php echo $i; ?>,<?php echo $row['ID']; ?>);">
										<?php while ($row_EstServLlamada = sqlsrv_fetch_array($SQL_EstServLlamada)) { ?>
											<option value="<?php echo $row_EstServLlamada['IdEstadoServicio']; ?>" <?php
											   if (isset($_GET["IdEstadoServicio"]) && ($_GET["IdEstadoServicio"] != "")) { // SMM, 12/09/2022
								   				if ($_GET['IdEstadoServicio'] == $row_EstServLlamada['IdEstadoServicio']) {
													   echo "selected";
												   }
											   } else {
												   if ((isset($row['EstadoServicio'])) && (strcmp($row_EstServLlamada['IdEstadoServicio'], $row['EstadoServicio']) == 0)) {
								...
								<td>
									<select id="CanceladoPor<?php echo $i; ?>" name="CanceladoPor[]"
										class="form-control IdCanceladoPor"
										onChange="ActualizarDatos('CanceladoPor',<?php echo $i; ?>,<?php echo $row['ID']; ?>);">
										<?php while ($row_CanceladoPorLlamada = sqlsrv_fetch_array($SQL_CanceladoPorLlamada)) { ?>
											<option value="<?php echo $row_CanceladoPorLlamada['IdCanceladoPor']; ?>" <?php
											   if (isset($_GET["IdCanceladoPor"]) && ($_GET["IdCanceladoPor"] != "")) { // SMM, 12/09/2022
								   				if ($_GET['IdCanceladoPor'] == $row_CanceladoPorLlamada['IdCanceladoPor']) {
													   echo "selected";
												   }
											   } else {
												   if ((isset($row['CanceladoPor'])) && (strcmp($row_CanceladoPorLlamada['IdCanceladoPor'], $row['CanceladoPor']) == 0)) {
													   echo "selected=\"selected\"";
												   }
								...
								<td><input size="15" type="text" id="Anexo<?php echo $i; ?>" name="Anexo[]" class="form-control"
										readonly value="<?php echo $row['AnexoOrdenServicio']; ?>"></td>
								<td>
									<?php // SMM, 12/09/2022
											$ComentariosCierre = "";
									...
									<textarea cols="70" maxlength="2000" id="ComentariosCierre<?php echo $i; ?>"
										name="ComentariosCierre[]" class="form-control ComentariosCierre"
										onChange="ActualizarDatos('ComentariosCierre',<?php echo $i; ?>,<?php echo $row['ID']; ?>);"><?php echo $ComentariosCierre; ?></textarea>

								</td>
								<td><input size="15" type="text" id="EstadoOrdenServicio<?php echo $i; ?>"
										name="EstadoOrdenServicio[]"
										class="form-control <?php if ($row['EstadoOrdenServicio'] == "Abierto") {
											echo "bg-danger";
										} else {
											echo "bg-primary";
										} ?>"
										readonly value="<?php echo $row['EstadoOrdenServicio']; ?>"></td>
								<td><span
								...
								<td><span style="white-space:normal; width: 200px;"
										class="<?php if ($row['Integracion'] == 0) {
											echo "badge badge-warning";
										} elseif ($row['Integracion'] == 1) {
											echo "badge badge-primary";
										} else {
											echo "badge badge-danger";
										} ?>"><?php echo $row['Ejecucion']; ?></span></td>
							</tr>
							<?php
							$i++;
						}
